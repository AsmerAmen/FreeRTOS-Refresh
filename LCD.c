...
extern void LCD_voidInit(char _port)
{
	g_LCD_DataPort = _port;
	// Control Pins:
	/* RS: A0, R\W: A1, E: A2 */
	...
	// Data Pins:
	/* Connected to Port D */
	DIO_voidSetPortDirection(g_LCD_DataPort, DIO_u8PortOp);
...
extern void LCD_voidWriteCommand(U8 command)
{
	...
	DIO_voidWritePin(LCD_RW,DIO_u8Low);
		...
		/* Read the command */
	DIO_voidWritePort(g_LCD_DataPort, command);
	//	PORTD = command;
...
extern void LCD_voidDisplay(U8 data)
{
	...
	DIO_voidWritePin(LCD_E,DIO_u8High);
		...
		/* Read the command */
	DIO_voidWritePort(g_LCD_DataPort, data);
